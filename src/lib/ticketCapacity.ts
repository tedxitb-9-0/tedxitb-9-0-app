...
/** Max regular-price orders (30 presale + 159 regular = 189 total capacity). */
export const TICKET_REGULAR_CAP = 159;
 ...
 * then regular pool (159). Total hard cap: 189 tickets.
...
export function resolveNextTicketOffer(counts: {
    ...
    // Hard total cap: never exceed 189 tickets
...
export function logTicketCapacityDebug(
    ...
    console.log(`[ticketCapacity] ${label}`, {
        ...(args.startOfTodayWib && {
            startOfTodayWib: args.startOfTodayWib.toISOString(),
        }),
        ...(args.dbRowCount !== undefined && { dbRowCount: args.dbRowCount }),
        ...(args.totalDbRowCount !== undefined && {
            totalDbRowCount: args.totalDbRowCount,
        }),
